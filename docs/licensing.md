...
| Component | License | Bundled in release builds? |
| --- | --- | --- |
| Shuttle app + wrappers | AGPL-3.0 | yes (this repo) |
| signal-cli | GPL-3.0 | yes |
| Embedded CPython (python-build-standalone) | MPL-2.0 / PSF | yes |
| GOWA / TDLib | MIT / BSL-1.0 | when fetch succeeds at build time |
| Matrix Client-Server API | Apache-2.0 (spec); server-dependent | HTTPS only, no extra binary |
| fbchat, instagrapi | BSD-3-Clause / MIT | via embedded Python env |

Full credits: [ATTRIBUTION.md](../ATTRIBUTION.md) and **Settings → Attributions** in the app.

## signal-cli (GPL-3.0)

There is no better maintained open option for Signal in a local sidecar today, so **release builds bundle signal-cli** as a separate executable. Shuttle talks to it over JSON-RPC; it is not linked into the Rust binary.

GPL-3.0 obligations for redistributors include preserving license text, copyright notices, and making corresponding source available. Upstream source: [github.com/AsamK/signal-cli](https://github.com/AsamK/signal-cli).
