...
A Ferdium-beating RSS measurement (sleep + `scripts/rss-sample.sh` exist; numbers still TBD), AI replies (opt-in, sanitized), more networks (X, iMessage, SMS, Google Chat, LinkedIn, …), real audio calls where a backend actually supports them, Android, and encrypting the message database at rest. Details: [roadmap.md](roadmap.md).
