...
What is still open after **0.2.x**. The unified inbox, Rust core, per-account SQLite, keyring, seven connectors, routing, backup, media/composer pieces, calls UI stubs, telemetry, and desktop packaging already exist. This document is the remaining product surface under Shuttle’s north star.
...
| 6. Media & composer | Players, PDF, basic image editor, rich text per channel | partial (composer, Giphy, emoji, contact media tabs landed) |
...
| 10. Calls | Audio first, then video — network-dependent | partial (protocol + CallPanel; backend coverage uneven) |
...
| 13. Resource budget | Beat Ferdium by 50–70% RSS on a comparable multi-account idle load | planned (benchmark + target set) |
| 14. Daily-driver chrome | Tray, quick switch, shortcuts, account sleep, search polish | planned |
...
| Hibernation unloads renderers (~5–10 MB stub each when it works) | Account **sleep** stops the sidecar; UI cost stays fixed |
...
1. **Account sleep / auto-hibernate** (see track 14) — stop idle sidecars after N minutes; wake on tray notification, quick-switch, or opening that account. Manual `disabled` already exists; sleep is temporary and auto-waking.
2. **Shared WhatsApp helper** — prefer one GOWA (or equivalent) process multiplexing multiple Shuttle WhatsApp accounts instead of one heavy helper per account, if the helper allows it safely on loopback.
...
5. **Measure in CI or a script** — optional `scripts/rss-sample.sh` (or docs recipe) that samples `shuttle` + child PIDs the same way the Ferdium run was done.

Re-benchmark after sleep + multi-account WA sharing land; update this section with Shuttle numbers when available.

---

## 14 — Daily-driver chrome and QoL

Lightweight alone is not enough — Shuttle should feel like the obvious daily client: responsive chrome, less friction, still easy to extend. Ferdium still leads on tray, shortcuts, and “always there” desktop feel; closing that gap without copying the webview model is the job.
...
```
1–5, 12     already landed (shell → routing + telemetry)
        ↓
13+14a      Account sleep + RSS measurement  ← resource bar
        ↓
14b         Tray + quick switch + shortcuts  ← daily-driver feel
        ↓
6           Media viewers / editor (can overlap with 14)
        ↓
7           Backup/restore polish
        ↓
...
- Account sleep state: last activity, sleep after minutes, wake reason (user / notification / rule) — catalog or `config.json`, not per-message.
